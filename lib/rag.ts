...
export async function getContext(userMessage: string, namespace: string) {
  try {
    ...
    const results = await store.similaritySearch(userMessage, 5, {
      namespace: namespace,
    });
    console.log("Top 5 chunks (preview):", results.map((doc) => doc.pageContent.substring(0, 50) + "..."));

    if (results && results.length > 0) {
      const combinedContext = results.map((doc) => doc.pageContent).join('\n\n');
      const formattedPrompt = await promptTemplate.format({ context: combinedContext });
    ...
    return { combinedContext: "", contextSystemMsg: { role: "system" as const, content: formattedPrompt }, pineconeFailed: true };
  }
  // If no results
  const formattedPrompt = await promptTemplate.format({ context: "" });
  return { combinedContext: "", contextSystemMsg: { role: "system" as const, content: formattedPrompt }, pineconeFailed: false };
}
